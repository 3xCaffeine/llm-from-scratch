# gpt impl with GQA and KV for memory savings
# standalone script
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, use_cache=False):
        ...
        keys = keys_base.repeat_interleave(
            self.group_size, dim=1
        )  # Shape: (b, num_heads, num_tokens, head_dim)
        values = values_base.repeat_interleave(
            self.group_size, dim=1
        )  # Shape: (b, num_heads, num_tokens, head_dim)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run GPT with grouped-query attention."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n_layers", type=int, default=12, help="Number of transformer blocks."
    )
    parser.add_argument(
        "--n_kv_groups", type=int, default=2, help="Number of key/value groups."
    )
    parser.add_argument(
