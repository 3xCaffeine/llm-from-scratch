...
def _():
    ...
    import json
    import os
    return Counter, deque, json, lru_cache, mo, os, re, tiktoken


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""bits and bytes""")
    return
...
def _(tiktoken):
    ...
    for _ in range(300):
        decoded = gpt2_tokenizer.decode([_])
        print(f"{_}: {decoded}")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""simple bpe impl""")
    return


@app.cell
def _(Counter, deque, json, lru_cache, re):
...
@app.cell
def _(tokenizer):
    def _encode (input_text:str):
        token_ids = tokenizer.encode(input_text,allowed_special="<|endoftext|>")
        return(token_ids)

    _token_ids=_encode("Jack embraced beauty through art and life.")
    print(_token_ids)
    _token_ids=_encode("Jack embraced beauty through art and life.<|endoftext|>")
    print(_token_ids)
    print(tokenizer.decode(_token_ids))

    for _token_id in _token_ids:
        print(f"{_token_id} -> {tokenizer.decode([_token_id])}")
    return


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""saving & loading the tokenizer""")
    return
...
def _(os, tokenizer):
    ...
    gpt2_dir = os.path.join(_script_dir, "gpt2")
    os.makedirs(gpt2_dir, exist_ok=True)
    tokenizer.save_vocab_and_merges(vocab_path=os.path.join(gpt2_dir, "vocab.json"), bpe_merges_path=os.path.join(gpt2_dir, "bpe_merges.txt"))
    return (gpt2_dir,)


@app.cell
def _(BPETokenizerSimple, gpt2_dir, os):
    # Load tokenizer
    _script_dir = os.path.dirname(os.path.abspath(__file__))
    _gpt2_dir = os.path.join(_script_dir, "gpt2")
    tokenizer2 = BPETokenizerSimple()
    tokenizer2.load_vocab_and_merges(vocab_path=os.path.join(_gpt2_dir, "vocab.json"), bpe_merges_path=os.path.join(gpt2_dir, "bpe_merges.txt"))
    return
